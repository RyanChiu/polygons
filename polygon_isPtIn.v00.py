...
def isPtIn():
    ...
    for pt in pts.itertuples(index=True, name='Pandas'):
        ...
        for pl in pls.itertuples(index=True, name='Pandas'):
            # print(getattr(pl, 'NAME') + "(rm 0,):") # for debug
            ...
            if (len(plpts) % 2 == 0):
                ...
                continue
            # print(plpts) # for debug
            # print(poly) # for debug
            if (IsPtInPoly(getattr(pt, 'LON'), getattr(pt, 'LAT'), poly)):
                line = (
                    getattr(pt, 'DP') + ";" + getattr(pt, 'AC_NUM') + ";"
                    + str(getattr(pt, 'LON')) + ";" + str(getattr(pt, 'LAT')) + ";"
                    + getattr(pl, 'SEC_NUM') + ";" + getattr(pl, 'DP') + ";" + getattr(pl, 'NAME')
                )
                # print(line)
                lines.append(line + "\n")
    ...
    endtime = time
    print(endtime.strftime('%Y%m%d-%H:%M:%S') + ", ends.")
